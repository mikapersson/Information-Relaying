    ...
    def animate(t):
        ...
        for k in range(K):
            ...
            if k == retrieving_agent or k in relaying_agents:
                ...
                # Draw Rcom circle
                circ = plt.Circle(pos, Rcom, color=agent_color, fill=False, linestyle='--', linewidth=2, alpha=0.6, zorder=2)
                ax.add_artist(circ)
...
if __name__ == '__main__':
    # Scenario parameters
    K = 10  # something weird with K=3 seed=1  (weird meetup point for retrieving agent and first relay agent)
    Rcom = 1.0
    R = (K+4)*Rcom
    Ra = 0.6*R
    ...
    animation_save_path = fr'Animations/dijkstra_baseline_animation_K{4}_seed{1}.gif'
