...
def plot_communication_range(save_plot=False, C_dir=1.0, SINR_threshold=1.0, phi=np.pi/3, jammer_pos=None):
    ...
    if save_plot:
        plot_dir = "Media/Figures/Communication"
...
def plot_communication_range_comparison(save_plot=False, SINR_threshold=1.0, phi=np.pi/3, 
    ...
    plt.xlim([-2.5, 1.3])  # Changed from [-3, 1.5] to [-2, 2] for more balanced view
    plt.ylim([-1.02, 1.12])
    ...
    # Map old labels to new formatted labels with Yes/No
    label_mapping = {
        'C_{\\text{dir}}=0,C_{\\text{jam}}=0': ('No', 'No'),
        'C_{\\text{dir}}=1,C_{\\text{jam}}=0': ('Yes', 'No'),
        'C_{\\text{dir}}=0,C_{\\text{jam}}=3': ('No', 'Yes'),
        'C_{\\text{dir}}=1,C_{\\text{jam}}=3': ('Yes', 'Yes')
    }
    ...
    for handle, label in zip(handles, labels):
        # Check if this is one of our communication range labels
        found = False
